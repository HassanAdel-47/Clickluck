    ...
    <div class="container">
        ...
        <div class="row align-items-center justify-content-between gy-4">
            <div class="col-xxl-4 col-xl-5 col-lg-6 text-lg-start wow fadeInLeft text-center" data-wow-duration="0.5s"
                data-wow-delay="0.3s">
                <div class="faq-thumb">
                    <img src="{{ getImage('assets/images/frontend/faq/' . @$content->data_values->image, '364x500') }}"
                        alt="image">
                </div>
            </div>
                    ...
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="h-{{ $loop->iteration }}">
                            <button class="accordion-button collapsed" data-bs-toggle="collapse"
                                data-bs-target="#c-{{ $loop->iteration }}" type="button" aria-expanded="false"
                                aria-controls="c-{{ $loop->iteration }}">
                                {{ __(@$item->data_values->question) }}
                            </button>
                        </h2>
                        <div class="accordion-collapse collapse" id="c-{{ $loop->iteration }}"
                            data-bs-parent="#faqAccordion" aria-labelledby="h-{{ $loop->iteration }}">
